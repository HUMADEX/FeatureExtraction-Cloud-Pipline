...
#asr_text_result = 'This will be the testing text for the linguistic feature extraction.'

print("video_name: "+video_name)
print("asr_text_result: "+asr_text_result)
...
def ProcessLanguageRawfromTranscriptions(pathanalysisdetailes = INDIVIDUAL_ANALYSIS_PATH, analysisfiles=INDIVIDUAL_ANALYSIS_FILE, detailanalysis_path= LANGUAGE_ANALYSIS_OUTPUT_PATH , detailanalysis_file = LANGUAGE_ANALYSIS_OUTPUT_FILE):
                
        languageprocessor = LanguageProcessor()
        language_analysis = {}; 
        
        LA = languageprocessor.ProcessTranscription(asr_text_result)
        language_analysis = LA
        
        filename = pathanalysisdetailes + (analysisfiles % (str(video_name)))
...
def VisualizeSummary(data, printout = False, hasdepressed = True, hasnondepressed = True, summaryfile = LANGUAGE_ANALYSIS_OVERALL_FILE, summarypath = LANGUAGE_ANALYSIS_OVERALL_PATH):
...
def CreateCSV(inputjson = LANGUAGE_ANALYSIS_OUTPUT_FILE, outputjson = LANGUAGE_ANALYSIS_SUMMARYCS_FILE, outputcsv = LANGUAGE_ANALYSIS_SUMMARY_CSV_FILE, summarypath = LANGUAGE_ANALYSIS_SUMMARY_PATH):
    ...
    print(csvheader)
    csvheader.remove("A2_Use_of_negatively_valanced_words")
...
print("+++++++++++++++++++++++++++++++++Create CSV++++++++++++++++++++++++++++++++++++++++++")
CreateCSV();
print("---Linguistic features extraction is ended in %s seconds ---" % (time.time() - start_time))
