...
@RestController
@RequestMapping("/fhir")
public class FHIRStorePractitioner {

    private static final String FHIR_SERVER_URL = "http://x.x.x.x:8080/fhir/Practitioner/";

    @PostMapping("/store-practitioner")
    public ResponseEntity<Object> updateTaskStatus(@RequestHeader("Authorization") String apiKey, Exchange exchange) {
        ...
        System.out.println("FHIRStorePractitionerResponse: " + questID);
        ...
        try {
            ...
            if (!updateResponse.getStatusCode().is2xxSuccessful() || updateResponse.getBody() == null) {
                errors.put("error", "Failed to update Practitioner with ID: " + questID);
                return new ResponseEntity<>(errors, HttpStatus.INTERNAL_SERVER_ERROR);
            }
